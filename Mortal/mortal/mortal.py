...
from common import filtered_trimmed_lines, load_torch
...
def main():
    ...
    state = load_torch(config['control']['state_file'], map_location=torch.device('cpu'))
    cfg = state['config']
    ...
    if 'tag' in state:
        tag = state['tag']
    elif 'timestamp' in state and state['timestamp'] is not None:
    ...
    if review_mode:
        ...
        if grp_cfg and grp_cfg.get('state_file') and grp_cfg.get('network'):
            grp = GRP(**grp_cfg['network'])
            grp_state = load_torch(grp_cfg['state_file'], map_location=torch.device('cpu'))
            ...
            matrix = torch.zeros((len(feature), 4, 4), dtype=torch.float32)
