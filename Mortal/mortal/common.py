...
def load_torch(src, map_location='cpu'):
    try:
        return torch.load(src, weights_only=True, map_location=map_location)
        ...
        dtype = np.dtype
        try:
            with torch.serialization.safe_globals([scalar, dtype]):
                return torch.load(src, weights_only=True, map_location=map_location)
        except pickle.UnpicklingError:
            if hasattr(src, 'seek'):
                src.seek(0)
            return torch.load(src, weights_only=False, map_location=map_location)

def recv_msg(conn: socket.socket, map_location='cpu'):
    rx = recv_binary(conn, 8)
    (size,) = struct.unpack('<Q', rx)
    rx = recv_binary(conn, size)
    return load_torch(BytesIO(rx), map_location=map_location)
