...
window.onload = () => {
    ...
    const observe = urlParams.get("observe");

    hostInput.value = host;
    portInput.value = port;
    usernameInput.value = username;
    if (observe !== null) {
        observerModeCheckbox.checked = ["1", "true", "yes"].includes(observe.toLowerCase());
    }

    if (["1", "true", "yes"].includes((urlParams.get("autoconnect") || "").toLowerCase())) {
