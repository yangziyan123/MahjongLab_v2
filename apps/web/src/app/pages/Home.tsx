...
import { BookOpenCheck, FileSearch, History, PlayCircle } from "lucide-react";
    ...
    <div className="flex min-h-screen flex-col bg-slate-50">
      <header className="border-b border-slate-200/80 bg-white/95 shadow-sm backdrop-blur">
        <div className="container mx-auto flex flex-col gap-4 px-4 py-6 sm:flex-row sm:items-center sm:justify-between">
          ...
          <nav className="flex items-center gap-1" aria-label="历史记录">
        ...
        <div className="mx-auto max-w-6xl">
          <div className="mb-12 grid items-stretch gap-8 md:grid-cols-2 lg:grid-cols-3">
            ...
            <Card className="flex h-full flex-col border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-md">
              ...
              <CardContent className="mt-auto">
                <Button asChild className="w-full" size="lg">
                  <Link to="/review/import">导入牌谱</Link>
                </Button>
              </CardContent>
            </Card>
            ...
            <Card className="flex h-full flex-col border-slate-200 bg-white shadow-sm transition-shadow hover:shadow-md">
              <CardHeader className="flex-1">
                <div className="mb-4 flex h-12 w-12 items-center justify-center rounded-lg bg-amber-100">
                  <BookOpenCheck className="h-6 w-6 text-amber-700" />
                </div>
                <CardTitle>经典牌谱训练</CardTitle>
                <CardDescription>从经典牌局的任意小局开始，在真实牌桌上继续完成决策。</CardDescription>
              </CardHeader>
              <CardContent className="mt-auto">
                <Button asChild className="w-full" size="lg">
                  <Link to="/classic">选择牌谱</Link>
                </Button>
              </CardContent>
            </Card>
          </div>
