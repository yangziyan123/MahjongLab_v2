...
import { ReviewDecisionAnalysisPanel } from "../../components/review/ReviewDecisionAnalysisPanel";
...
export function ReviewReplay() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-950 xl:h-screen xl:overflow-hidden">
      <header className="sticky top-0 z-10 border-b border-slate-200 bg-white shadow-sm xl:static xl:h-[72px]">
        <div className="mx-auto h-full max-w-[1800px] px-4 py-3">
          <div className="flex h-full flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <h1 className="text-xl font-bold text-slate-900">逐步复盘</h1>
      ...
      <main className="mx-auto max-w-[1800px] px-4 py-4 xl:h-[calc(100vh-72px)] xl:overflow-hidden">
        <div className="grid gap-4 xl:h-full xl:min-h-0 xl:grid-cols-[260px_minmax(0,1fr)_380px]">
          <Card className="h-fit border-slate-200 bg-white shadow-sm xl:h-full xl:min-h-0 xl:gap-0 xl:overflow-hidden">
            <CardHeader className="px-4 pt-4 xl:shrink-0">
            ...
            <CardContent className="space-y-4 px-4 xl:flex xl:min-h-0 xl:flex-1 xl:flex-col xl:pb-4">
              ...
              <div className="max-h-[520px] space-y-4 overflow-y-auto pr-2 xl:min-h-0 xl:flex-1 xl:max-h-none">
                ...
                <div className="shrink-0 rounded-xl border border-slate-200 bg-white p-3 shadow-sm">
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="font-semibold text-slate-900">
                        {formatKyokuLabel(selectedEntry.kyoku_index, selectedEntry.honba)}
                      </span>
                  ...
                  <div className="mt-3 grid gap-2 md:grid-cols-[1fr_auto_1fr] md:items-center">
                    <div className="flex items-center justify-between rounded-lg bg-rose-50 px-3 py-2">
                      <span className="text-xs font-medium text-rose-700">你的动作</span>
                      <span className="font-bold text-slate-950">{formatAction(selectedEntry.actual_action)}</span>
                    </div>
                    <ArrowRightCircle className="mx-auto hidden h-4 w-4 text-slate-400 md:block" />
                    <div className="flex items-center justify-between rounded-lg bg-emerald-50 px-3 py-2">
                      <span className="text-xs font-medium text-emerald-700">AI 推荐</span>
                      <span className="font-bold text-slate-950">{formatAction(selectedEntry.expected_action)}</span>
                    </div>
                  </div>
                  <p className="mt-2 text-xs leading-5 text-slate-500">{getDeviationSummary(selectedEntry)}</p>
                </div>

                <div className="flex min-h-[420px] flex-1 items-center justify-center overflow-hidden rounded-xl bg-slate-100 p-2 xl:min-h-0">
                  <MahjongAiReviewFrame
                    entry={selectedEntry}
                    targetPlayerLabel={review.target_player_label}
                    fitViewport
                  />
                </div>
              </section>

              <ReviewDecisionAnalysisPanel
